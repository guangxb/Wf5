...
namespace Sys.Workflow.Business
{
    ...
    public class ProcessModel
    {
        ...
        internal XmlDocument XmlProcessDefinition
        {
            get
            {
                if (XpdlCachedHelper.GetXpdl(ProcessGUID) == null)
                {
                    var xmlDoc = GetXmlProcessDefinitionFromDisk();

                    XpdlCachedHelper.SetXpdl(ProcessGUID, xmlDoc);
                }
                return XpdlCachedHelper.GetXpdl(ProcessGUID);
            }
        }
        ...
        {
            Debug.WriteLine("read file once...");
        ...
        internal NextActivityMatchedResult GetFirstActivityList(IDictionary<string, string> conditionKeyValuePair)
        {
            ...
            {
                ActivityEntity startActivity = GetStartActivity();
                return GetNextActivityList(startActivity, conditionKeyValuePair);
            }
        ...
        /// <returns></returns>
        internal IList<NodeView> GetNextActivityTree(Guid currentActivityGUID, IDictionary<string, string> condition = null)
        {
            var activity = GetActivity(currentActivityGUID);
            var nextSteps = GetNextActivityList(activity, condition);
