...
namespace Sys.Workflow.Engine
{
    ...
    internal class WfRuntimeManagerAppRunning : WfRuntimeManager
    {
        internal override void ExecuteInstanceImp(ISession session)
        {
            ...
            {
                throw new WfRuntimeException(string.Format("当期流程不在运行状态！详细信息：当前流程状态：{0}", runningExecutionContext.ProcessInstance.ProcessState));
            }

            if (!(base.TaskView.ActivityState == (short)NodeStateEnum.Ready 
                || base.TaskView.ActivityState == (short)NodeStateEnum.Running))
            {
